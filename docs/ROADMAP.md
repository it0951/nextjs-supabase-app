...
- [x] 잘못된/만료된 invite_token으로 접근 시 적절한 에러 페이지 노출
- [x] 참여자 등록 시 join_token이 고유하게 생성됨
- [x] join_token이 없으면 참여자 뷰 페이지 접근 불가 (RLS 동작)
- [x] 카풀 그룹 배정/해제가 즉시 양쪽 화면(주최자/참여자)에 반영됨
- [x] 정산 항목 추가 시 모든 확정 참여자에게 균등 분배되어 자동 split 생성
- [x] 납부 여부 토글이 주최자/참여자 모두에게 정상 반영됨
- [x] Playwright E2E: 비회원 전체 플로우 + 카풀/정산 플로우 통과
